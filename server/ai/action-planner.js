...
function parseCreateTitle(question, entityLabel) {
  const text = cleanText(question);
  const pattern = new RegExp(`(?:新建|创建|新增|添加)(?:一个|一条|个|条)?${entityLabel}[：:\\s]*[“\"]?(.+?)[”\"]?(?=(?:，|,|；|;|。|\\s+(?:优先级|状态|进度|截止|分类|内容)[：:]?)|$)`);
  return trimPayloadText(text.match(pattern)?.[1] || '').slice(0, 160);
}

function parseLabeledText(question, label) {
  const match = cleanText(question).match(new RegExp(`${label}[：:]\\s*(.+?)(?=(?:，|,|；|;|。|\\s+(?:标题|分类|内容|下一步|耗时|工时|进度)[：:]?)|$)`));
...
function taskMatchScore(task, question) {
...
async function resolveTask(question, preferredTaskId = null) {
  ...
  const matches = tasks
    .map((task) => ({ task, score: taskMatchScore(task, question) }))
...
async function resolveNote(question) {
  ...
  const matches = rows
    .filter((row) => compactText(row.title) && compactText(question).includes(compactText(row.title)))
    .sort((a, b) => compactText(b.title).length - compactText(a.title).length);
...
function actionConfirmation(action, sources = [], intent = 'action') {
  return {
    ...
    answer: [
      ...
      '<h3>待审批操作已生成</h3>',
      `<p>我已经根据你的指令生成了 <strong>${escapeHtml(action.title)}</strong>。确认前不会修改任务台数据。</p>`,
      '</section>',
    ].join(''),
    sources,
    grounded: true,
...
async function planTaskAction(question, options) {
  const text = cleanText(question);
  const createIntent = /(?:新建|创建|新增|添加)(?:一个|一条|个|条)?任务/.test(text);
  if (createIntent) {
  ...
  if (Object.keys(payload).length === 1) {
    return clarification('已经找到目标任务，但还需要你说明要修改状态、进度、优先级、截止日期或说明中的哪一项。', 'action_update_task');
  }
...
async function planLogAction(question, options) {
  ...
  if (createIntent) {
    ...
    if (!task) return clarification('请写明这条日志属于哪个任务。', 'action_create_log');
...
async function planNoteAction(question, options) {
  ...
  if (createIntent) {
    ...
    const sources = resolvedTask.task ? [sourceForTask(resolvedTask.task)] : [];
    return actionConfirmation(action, sources, 'action_create_note');
  }
